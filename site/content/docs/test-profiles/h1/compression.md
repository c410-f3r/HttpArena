...
title: Compression
---
{{< type-rules production="Must use the framework built-in gzip compression. No pre-compressed caches, no custom compression libraries." tuned="May use alternative compression libraries, pre-compressed responses, or hardware-accelerated compression." engine="No specific rules." >}}
...
2. **Configurable compression level** - the framework must allow setting gzip to its fastest level (level 1). Frameworks that hardcode the compression level and do not expose configuration are excluded, since compression level has a significant impact on both throughput and output size.
