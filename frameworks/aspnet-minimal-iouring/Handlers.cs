...
static class Handlers
{
    ...
    public static Results<JsonHttpResult<ResponseDto<DbResponseItemDto>>, ProblemHttpResult> Database(HttpRequest req)
    {
        ...
        var query = req.Query;
        if (query.TryGetValue("min", out var minStr) && int.TryParse(minStr, out var pmin)) min = pmin;
        if (query.TryGetValue("max", out var maxStr) && int.TryParse(maxStr, out var pmax)) max = pmax;
    ...
    public static async Task<Results<JsonHttpResult<ResponseDto<DbResponseItemDto>>, ProblemHttpResult>> AsyncDatabase(HttpRequest req)
    {
        ...
        int limit = 50;
        var query = req.Query;
        if (query.TryGetValue("min", out var minVal) && int.TryParse(minVal, out var pmin)) min = pmin;
        if (query.TryGetValue("max", out var maxVal) && int.TryParse(maxVal, out var pmax)) max = pmax;
